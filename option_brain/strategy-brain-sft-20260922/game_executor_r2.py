...
sys.path.insert(0,str(Path(__file__).resolve().parent.parent/'strategist-rl-king-continuation-20260921'))
"""Public-observation RL adapter and continuous manual actuator. No old macros."""
import importlib.util, sys, time
import numpy as np
import torch
from config import *
# Derived candidate: native tick/request budgets are authoritative, not a hidden worker cap.
...
class Executor:
    ...
    def fight(self,g):
        ...
        while s.state['tick']-start<g.get('max_ticks',2000):
            ...
            if h['hp_fixed']<hp_at_start:return 'damage_received'
    ...
    def service(self,g):
        ...
        for index,command in enumerate(g['commands']):
            ...
            if command['kind']=='move':
                ...
                continue
            wait_hp=self.s.state['hero']['hp_fixed']
            wait_scene=scene(self.s.state)
            wait_seen={e['id'] for e in self.s.state['enemies']}
            for _ in range(60):
                if idle(self.s.state) or command['kind'] in ('drink','dismiss'):break
                if self.s.state['hero']['dead']:return 'dead'
                self.s.step()
                if self.s.state['hero']['hp_fixed']<wait_hp:return 'damage_received'
                if scene(self.s.state)!=wait_scene:return 'scene_changed'
                if {e['id'] for e in self.s.state['enemies']}-wait_seen:return 'new_enemy'
