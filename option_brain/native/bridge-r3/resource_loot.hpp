...
bool ResourceLootUpgrade(const Player &player, const Item &item, bool protectRepairPotential = true)
{
	...
	if (PlanGearUpgrade(player, item, false, true).valid)
		return true;
	if (!gGearWearScoring || !protectRepairPotential || item.isEmpty() || !item.isEquipment())
		return false;
	// A repairable future upgrade is not sale stock. These are detached copies;
	// the actual equip/buy/repair paths still require present stats and payment.
	Player repaired = ResourceUnequipSimulation(player);
	for (Item &equipped : repaired.InvBody) {
		if (!equipped.isEmpty() && equipped._iMaxDur > 0
		    && equipped._iMaxDur != DUR_INDESTRUCTIBLE)
			equipped._iDurability = equipped._iMaxDur;
	}
	Item restored = item;
	if (restored._iMaxDur > 0 && restored._iMaxDur != DUR_INDESTRUCTIBLE)
		restored._iDurability = restored._iMaxDur;
	CalcPlrInv(repaired, false);
	SetPlrAnims(repaired, true);
	return PlanGearUpgrade(repaired, restored, false, true).valid;
}
...
py::dict ResourceLootItemState(const Item &item, int index)
{
	...
	const bool upgrade = price.has_value() && ResourceLootUpgrade(*MyPlayer, item, index >= 0);
...
void AppendResourceLootState(py::dict &result, py::dict &town)
{
	...
	result["max_service_trips"] = gResourceTripLimitsEnabled ? py::cast(MaxLootServiceTrips) : py::none();
	...
	for (int i = 0; i < ActiveItemCount; ++i) {
		...
		if (!IsTileLit(item.position) || !ResourceLootSellPrice(item).has_value()
		    || ResourceLootUpgrade(*MyPlayer, item, false)) continue;
    ...
    uint16_t seedHigh, uint16_t seedLow, uint16_t createInfo, int baseId)
{
	...
	if (ResourceLootUpgrade(*MyPlayer, floor, false)) return receipt(false, "reserved_upgrade");
