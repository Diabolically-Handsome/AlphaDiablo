...
## 判决附录(2026-07-12 21:16 案结;本附录 commit 即公证 = 课程战役解锁时刻)

冻结 commit `50e356ef7c9d43044bc6af1b60059be4da59538a`;台账
`train/runs/reanchor-r2/gate_ledger.jsonl`;点火 21:08:53,案结 21:15:59
(**五发全程 7 分钟**,每发 ≈70-100 秒,系 R2 后预算校准依据)。五发
全部一发即中(零 -b、零 OPERATIONAL、驱动器退出码 0 零瑕疵)。

**锚值对照表(A1-A5 固定顺序,禁按数值排序;非名次、非挑战、不改名分)**

| 发 | v3 金池读数 | v2 前科(池别) | 对照栏 | 档案 sha16 |
|---|---|---|---|---|
| A1 科学锚 | **152.8**(中位 153.1,死 6/32,深度中位 **2.0**,kills 61.1) | 140.3(7000 池) | 跨世界×跨池,禁位移解读 | 8d6a05d3517a6481 |
| A2 发射锚·现任 | **133.9**(中位 133.4,死 4/32,深度中位 1.0) | 112.4(7000 池) | 跨世界×跨池,禁位移解读 | 8ab6b51065105a67 |
| A3 王座 | **97.7**(中位 93.8,死 2/32,kills 41.2) | 97.2(金池) | **同池跨世界:Δ=+0.5** | 2324648a416cbc0c |
| A4 脚本参照 | **94.2**(中位 103.05,死 2/32) | 93.9(金池,leaderboard 在册旁证) | **同池跨世界:Δ=+0.3** | 71c298e05b6bf19e |
| A5 教师组装考核(非锚) | 94.2(死 2/32) | 无前科 | 闸门发 | 0f5970acc26f200e |

- **锚之授予**:`v3-science-anchor := r2-science.json`(sha
  `8d6a05d3…e273f5`);`v3-launch-anchor := r2-launch.json`(sha
  `8ab6b510…42dff1`)。**王座在位锚值 := 97.7;脚本参照值 := 94.2**。
  名分零变动。ANCHOR_GRANT 事件含消费所需全字段(archive_path/sha/
  worker/manager 身份/seeds/freeze_sha),后续驱动器禁按 tag 猜路径。
- **R-O:预测成立**(152.8 ≥ 133.9 ≥ 94.2);王座与发射锚相对序未预测,
  实测发射 133.9 > 王座 97.7,只记不裁。
- **R-Δ(同池跨世界,唯二合法位移读数)**:王座 +0.5、脚本 +0.3——
  七补丁 + 回跳禁闭对**浅层生活阵容**的金池测量近乎零扰动,系"v2 同引擎
  配对结论内部有效"之最硬佐证;世界改动的咬合面在深层经济(A1 深度中位
  1.0→2.0、死亡 3→6 类形变,因跨池仅作观察不作位移)。
- **R-G1:PASS**(94.2 ≥ 80.07 = 0.85×94.2 之线)。且系**完美克隆**:
  32 局 42,571 次部署调用 worker_divergences=0,agg 与 A4 逐位相同——
  R1 之 G1-数据侧 top1=1.000 在组装金池口径下兑现为行为等价。R1 顺延
  义务清偿;判词限定解除:"学习工人(BC 教师形态)可在金池组装口径
  平替脚本"自此有据。该发读数永不称锚。
- **档案入库**:五档 + 榜 `train/leaderboard-assembled-v3.md` 随本附录
  commit `git add -f` 收录;ANCHOR_GRANT 所引 sha 在册可复验。
- **序列完成宣告**:外源审计三步序列(BC 重生成 → 教师重铸 → 定锚重测)
  随本 commit 全部落账——**课程战役解锁,A 复赛权可动用**(ROADMAP
  时代切换注记之封条自此解除)。
