...
## 判决附录(2026-07-12 20:27 案结;本附录 commit 即公证,落账后不可改)

冻结 commit `106eb16954c5ba55f304acaea41587cc5c9785bf`;台账
`train/runs/reanchor-r1/gate_ledger.jsonl`;发车 20:14:28,案结 20:27:24
(**全程 13 分钟**——"预计时长系估算"条款兑现:S1 实测 5.6 分 / S2 3.2
分 / S3 4.1 分,此为 R2 及后续案预算校准依据)。案级判词:**案结**
(R-W PASS ∧ R-S 三份全过)。三条 R 线预测**全部命中**。

- **R-W:PASS(预测命中)**。pairs=166383,held-out top1=**1.000**,
  门槛类召回 {9: 1.0},零重试(class_weighted_retry=false)。回执:
  policy `f052067a589cfcdedaf1754ae6d241d736bb97f6fc798683f395c76cb0ff98e6`、
  demos `3bf892d611e41853eca8fce0cb146753af41ad2c3a21b6c581df1041fb1d9363`、
  manager_npz = W7 台账值、impl `76dba1eb…`、generator `505ade6f…`
  (全文见台账)。**v3 教师(数据侧)就位**;G1 判词限定生效:组装考核
  顺延 R2,本判决不宣称"学习工人可平替脚本"。观察入册:门槛类由 v2 之
  {9,10} 缩为 {9}(10 号动作全集样本 <300)——示范分布随世界实变,
  重生成非例行公事。
- **R-M:FAIL(预测命中)**。ratio=**0.6478**(51.124/78.922)。判词照
  D3 注册文本:在注册配方下经理教师不可被 303 维无记忆 BC 以 ≥0.85
  克隆(v3 世界);机制候选 = env.exhausted 包装器状态不可观测;除名
  依据为工程不可用性。**跨世界观察**:与 v2 前科 0.6498(同池口径)近
  逐位复现——候选解释 (a)"v2 世界缺陷所致"被强烈削弱,教师状态依赖的
  世界不变性上位(此系观察,不改注册判词)。M-BC 臂 v3 时代除名,选举
  fresh-only;复活须教师改造案。
- **R-F:FAIL(预测命中;"置信度低"条款如实兑现)**。ratio=**0.4786**
  (18.758/39.198)——平面教师首个**同池口径**读数(v2 之 0.5974 系
  有偏旧口径,不可比,见案由)。平面 BC 基线 v3 时代除名;机制候选 =
  停滞钟 _clock 包装器状态不可观测。
- **前科 ↔ _previous 对照表(W8 义务)**:bc-worker{report `f7f825b3…`,
  demos `91d9aec3…`, policy `4de43826…`}→ `_previous/1783858471165715000`;
  bc-manager{report `374760c4…`, policy `0772d85e…`}→
  `_previous/1783858810262880000`;bc-flat{report `5909671a…`, policy
  `1f4b5f0c…`}→ `_previous/1783859000643200000`。字节保全,全文 sha 见
  台账 PREV_ARTIFACT 事件。
- **事故记录(不溯罪)**:驱动器顶层守卫误捕 `SystemExit(0)`,于
  CASE_CLOSED 之后留下一条假 DRIVER_EXCEPTION 台账事件与 NEEDS_ATTENTION
  条目;判无实害(案已结、真实退出码 0)。热修:`sys.exit` 移出 try
  (本附录同 commit),NEEDS_ATTENTION 已追加处置注记。
- **序列进度**:ROADMAP 三步之前两步(BC 基底重生成 + 教师重铸)随本案
  完成;下一案 **R2 定锚**(金种子重测,须新预注册 + 总设计师知会)。
