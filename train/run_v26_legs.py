...
SPS_FLOOR/tail_cut 禁用(降档机制全案只许一套:任一腿墙钟 >30 分钟 → STOP);
...
def main():
    ...
    while k <= N_LEGS:
        ...
        # ---- G-绿洲(仅腿 1):学习窗 dry≡0 且 ff_dry>0(PREREG-v26 G 线)----
        if k == 1:
            ...
            if lines:
                ...
                log({"event": "g_oasis", "dry": last.get("dry"),
                     ...
                     "ok": oasis_ok})
                if not oasis_ok:
                    log({"event": "STOP", "why": "G-绿洲失败:学习窗含 dry 或 ff_dry=0"})
                    return
    ...
    for kk, (sc16, mp, bt) in top2:
        agg = exam(pathlib.Path(mp), f"v26-G3-leg{kk}", "7000-7031")
        if agg is None:
    ...
    paired = {}
    for kk, (sc16, mp, bt) in top2:
        rows = json.loads((RUNS / "eval-assembled" / f"v26-G3-leg{kk}.json").read_text())["rows"]
    ...
    if not qual:
        best_pd = max(paired.values(), key=lambda x: x[0])[0] if paired else 0
        band = "探针级改进 [+2,+4),不烧牌留工作站复赛" if best_pd >= 2.0 else "连任,本轮无增益"
