"""R8 certification campaign launcher (opened 2026-07-28; approved: accept the R7 verdict and start R8,
with 4-core certification purity (the recommended option)). Derived from run_r7_combat_recovery.py (rev23,
commit 0129d5f): the amendment 5/6 adoption machinery (specific to R7 history) is stripped,
single-recipe risk64 replication certification, gate B made native, death yardstick switched to the exact conditional McNemar
(r8_statistics, margin 0.10). Full design: docs/prereg/PREREG-R8-certification.md
"""
...
STATE_SCHEMA = "diablogym-r8-campaign-state/1"
TRAINING_RECEIPT_SCHEMA = "diablogym-r8-training-artifact/9"
DRY_CURRICULUM_LEDGER_SCHEMA = "diablogym-r8-dry-curriculum-ledger/1"
TRAINING_FIRED_SCHEMA = "diablogym-r8-training-fired/1"
DEVELOPMENT_DECISION_SCHEMA = "diablogym-r8-development-decision/1"
FINAL_OPENED_SCHEMA = "diablogym-r8-final-pool-opened/1"
FINAL_FIRED_SCHEMA = "diablogym-r8-final-candidate-fired/1"
EVAL_FIRED_SCHEMA = "diablogym-r8-eval-fired/1"
...
# R8 replication gate (PREREG-R8 §2; R7 amendment 5 gate B inherited as the native criterion):
# per leg = the pre-registered check set minus the single item deaths.noninferiority_upper_bound
# (observed_not_higher kept); the death NI is computed and archived as usual, record only,
# and non-inferiority inference belongs to the final exam alone.
R8_DEVELOPMENT_GATE = (
    "prereg-checks-minus-deaths.noninferiority_upper_bound")
R8_DEVELOPMENT_DROPPED_CHECK = "deaths.noninferiority_upper_bound"
# Amendment R8-1 (2026-07-29, approved: file the amendment and resume): the replication gate's original
# observed_not_higher limb is a per-pool point estimate of death counts; under the null truth the single-pool pass rate is ~53%
# (a coin flip), ~28% for both pools, and the total power of 2/3 over three legs ~19% -- nearly unreachable before any candidate data,
# the fourth death-limb power defect of the same kind. Changed to a catastrophe band over both pools combined: per leg
# (2 pools, 256 pairs) excess deaths ≤ +5pp. Per-leg power under the null truth 91-95%
# (sd=√D_pooled/256≈3.0-3.5pp, frozen D_pooled=73/60/79; +5pp≈1.4-1.7σ),
# gate level (≥2/3 legs) ~98.6%; still blocks real regressions of ≥+8pp. The final-exam criteria
# (McNemar NI + observed_not_higher) are unchanged.
...
PUBLISHED_DIR = TRAIN / "runs" / "r8-certification-published"
...
TERMINAL_DEATH_EVIDENCE_SCHEMA = (
    "diablogym-r8-terminal-death-reward-evidence/3"
)
...
# R8 pool ledger: 2_110/2_111 (R7 development), 2_120 (burned in the incident), 2_121 (R7 final exam)
# used; this case takes 2_112/2_113 (replication) and 2_122 (final exam); 13 ranges remain.
DEV_POOLS = {
    "dev-a": tuple(range(2_112_000, 2_112_128)),
    "dev-b": tuple(range(2_113_000, 2_113_128)),
}
FINAL_POOL = tuple(range(2_122_000, 2_122_256))
DEVELOPMENT_TRAIN_SEEDS = (2_131_000, 2_131_100, 2_131_200)
PRODUCTION_TRAIN_SEED = 2_131_900
...
# R8: recipe selection was completed by R7 (amendment 5 gate B, risk64 2/3 over both pools).
# This is a certification campaign with single-recipe replication -- 3 fresh training seeds must pass gate B on both pools in ≥2/3.
RECIPES = {
    "risk64": {"additional_terminal_death_cost": 64.0},
}
RECIPE_PREFERENCE = ("risk64",)
MIN_REPLICATIONS_PASSING_BOTH_POOLS = 2
DEVELOPMENT_DEATH_MARGIN = 0.10
# R8 (PREREG-R8 §1.3): the death yardstick switches to the exact conditional McNemar (r8_statistics),
# margin 0.10 unchanged. Power (40k simulations, α=0.005): null truth 0.76, development truth
# 0.95; calibration vector: the R7 final exam (c=31, D=62, n=256) → UCB 0.0805.
FINAL_DEATH_MARGIN = 0.10
FAMILYWISE_ALPHA = 0.05
GEAR_PROGRESSION_GATE_SCHEMA = (
    "diablogym-r8-action14-gear-progression-gate/2"
)
...
METRIC_RULES = (
    r8_statistics.MetricRule("farm_worker_wage"),
...
def _recipe_document() -> dict:
    return {
        "schema_version": "diablogym-r8-recipe/1",
        ...
        "development_gate": R8_DEVELOPMENT_GATE,
        ...
        "statistics": {
            "schema": r8_statistics.R8_STATISTICS_SCHEMA,
            "method": r8_statistics.R8_METHOD_REVISION,
            "metric_rules": [
                {
                    "key": rule.key,
...
def _implementation_identity() -> dict:
    ...
    return {
        ...
        "statistics_sha256": _sha256(TRAIN / "r8_statistics.py"),
...
def _run_name(recipe: str, seed: int, scope: str) -> str:
    ...
    return f"r8-{recipe}-{suffix}-s{seed}"
...
def command_train_development() -> None:
    ...
    _require(
        ...
        "amendment R8-1 adoption is in effect; the replication training stage is sealed, rerun forbidden",
    )
    bc = _bc_identity()
    _require(
...
def _eval_tag(pool: str, arm: str, recipe: str | None = None,
    ...
    if pool == "final":
        return f"official-r8-final-{arm}-{FINAL_POOL[0]}"
    if arm == "baseline":
        return f"r8-{pool}-baseline-v28"
    _require(recipe is not None and seed is not None, "development candidate tag lacks identity")
    return f"r8-{pool}-{recipe}-s{seed}"
...
REV21_DIAGNOSTICS_SCHEMA = "diablogym-r8-rev21-diagnostics/1"
...
def _analyze_pair(
    ...
    analysis = r8_statistics.analyze_paired_archives(
        baseline,
        candidate,
...
def _replication_leg_passes(analysis: dict) -> bool:
    ...
    _require(
        ...
        and R8_DEVELOPMENT_DROPPED_CHECK in checks,
        "replication analysis verdict internally inconsistent",
    )
    return not [
        name for name in failed
        if name != R8_DEVELOPMENT_DROPPED_CHECK
    ]
    ...
    for recipe in RECIPE_PREFERENCE:
        ...
        for seed in DEVELOPMENT_TRAIN_SEEDS:
            by_seed[str(seed)] = all(
                _replication_leg_passes(analyses[f"{pool}:{recipe}:{seed}"])
                for pool in DEV_POOLS
            )
...
def _validate_development_decision(state: dict) -> dict:
    # Amendment R8-1: in the adopted state the replication artifacts are bound to the rev1 identity, so per-analysis recomputation must mismatch;
    # use the frozen re-check + an equality check of the new-gate re-derivation instead. A campaign that was not adopted takes the original path.
...
def command_eval_development() -> None:
    ...
    _require(
        ...
        not in state["phases"].get("eval_development", {})
        and not R8A1_PATH.exists(),
        "amendment R8-1 adoption is in effect; the replication evaluation stage is sealed, rerun forbidden",
    )
    _require(
...
def _r8a1_pooled_death_excess(
    ...
    for pool in DEV_POOLS:
        ...
        _require(
            ...
            "frozen analysis death accounting fields invalid",
        )
...
def _r8a1_selection(analyses: dict[str, dict]) -> dict:
    _require(set(analyses) == set(_development_analysis_keys()),
             "r8a1 analyses key set differs from the frozen plan of 6")
    derivation = {}
    per_recipe = {}
    for recipe in RECIPE_PREFERENCE:
        passing = []
...
def _r8a1_require_judgment_anchor(pre: dict) -> None:
    ...
    _require(
        ...
        "the 6 analyses have lost their anchor to decision.analysis_sha256s at the decision moment",
    )
    for tag in _r8a1_eval_tags():
...
def _r8a1_require_rev1_terminal(state: dict) -> None:
    ...
    _require(
        ...
        "r8a1 can only adopt the rev1 DEVELOPMENT_SCIENTIFIC_FAIL terminal state",
    )
    _require(
        ...
        "rev1 scientific-fail phase record not closed",
    )
    _require(
        state.get("phases", {}).get("train_development", {}).get("status")
        == "complete",
        "replication cohort not completely frozen; cannot adopt",
    )
...
def _r8a1_repair_crash_window() -> None:
    ...
    _require(
        ...
        "crash-window adoption: the existing r8a1 document cannot be re-checked against the rev1 terminal state",
    )
    _r8a1_migrate_state(state, pre, selection)


def _validate_r8a1_adoption(state: dict) -> dict:
    _require(R8A1_PATH.exists(), "r8a1 adoption file missing")
    document = _stable_json(R8A1_PATH)
    phase = state["phases"].get("eval_development", {})
    _require(
        set(phase) == {
    ...
    _require(
        ...
        "r8a1 document identity not closed",
    )
    _require(
    ...
    _require(current == pre, "r8a1 frozen artifact inventory drift")
    _r8a1_require_judgment_anchor(pre)
    selection = _r8a1_selection(_r8a1_read_frozen_analyses())
    _require(post.get("selection") == selection,
             "r8a1 selection cannot be recomputed from the frozen analyses")
    _require(
        selection["selected_recipe"] in RECIPES
        and phase["selected_recipe"] == selection["selected_recipe"],
        "r8a1 selected_recipe not closed",
    )
...
def command_adopt_replication() -> None:
    ...
    if R8A1_PATH.exists():
        ...
        print("r8a1 adoption already on record and re-checked; selected_recipe="
              + verdict["selected_recipe"])
        return
    _require(STATE_PATH.exists(), "no state; r8a1 can only adopt the rev1 terminal state")
    state = _stable_json(STATE_PATH)
    ...
    _require(
        selection["selected_recipe"] in RECIPES,
        "no recipe reaches 2/3 replication under the r8a1 gate either; refusing to adopt",
    )
    document = {
        ...
        "approval": {
            ...
            "wording": "file the amendment and resume (approved 2026-07-29)",
            "date": "2026-07-29",
        },
        "justification": (
            "the observed_not_higher limb of the replication gate is a per-pool point estimate of deaths; under the null truth"
            " the single-pool pass rate is ~50-58% (a coin flip), and the total power of 2/3 over three legs ~19% -- nearly"
            " unreachable before any candidate data (the fourth death-limb power defect of the same kind; pure algebra, re-checkable"
            " offline, docs/prereg/PREREG-R8-certification.md, section amendment R8-1). Changed to"
            " a catastrophe band over both pools combined (excess deaths ≤ +5pp; per-leg power under the null truth 91-95%,"
            " gate level ~98.6%); final-exam criteria unchanged. The final-exam pool 2_122 is unconsumed and unread,"
            " so the certification decision is not contaminated."
        ),
    ...
    print("r8a1 adoption complete (labelled post-hoc); selected_recipe="
          + verdict["selected_recipe"]
...
def _validate_final_registry_document(
    ...
    _require(
        ...
        # the global registry is shared across campaigns: R7's historical consumption records (r7-official-final,
        # pools 2_120/2_121) live in the same book as this case's records; both names are allowed, overlapping seeds are still refused.
        and record["pool_name"] in {"r7-official-final", "r8-official-final"}
