"""R8 认证战役 launcher(2026-07-28 立案,批文「接受并且开始R8」+
...
# R8 复现门(PREREG-R8 §2,继承 R7 修正案五 B 门为原生判据):
# 逐腿 = 预注册检查集减去 deaths.noninferiority_upper_bound 单项
# (observed_not_higher 保留);死亡 NI 照常计算入档,只记不裁,
# 非劣推断专属终考。
R8_DEVELOPMENT_GATE = (
...
# R8 池台账:2_110/2_111(R7 开发)、2_120(事故烧)、2_121(R7 终考)
# 已耗;本案划 2_112/2_113(复现)与 2_122(终考);余 13 段。
DEV_POOLS = {
...
# R8:配方选拔已由 R7 完成(修正案五 B 门,risk64 2/3 双池)。
# 本案为认证战役,单配方复现——3 个全新训练种子须 ≥2/3 双池过 B 门。
RECIPES = {
...
# R8(PREREG-R8 §1.3):死亡尺换精确条件 McNemar(r8_statistics),
# 边距 0.10 不变。功效(40k 模拟,α=0.005):持平真值 0.76、开发真值
# 0.95;校准向量:R7 终考实况 (c=31,D=62,n=256) → UCB 0.0805。
...
def _replication_leg_passes(analysis: dict) -> bool:
    ...
    _require(
        ...
        "replication analysis verdict 内部不一致",
    )
    return not [
...
def _validate_final_registry_document(
    ...
    _require(
        ...
        # 全局 registry 跨战役共用:R7 的历史消费记录(r7-official-final,
        # 池 2_120/2_121)与本案记录同居一册,双名放行、种子重叠仍拒。
