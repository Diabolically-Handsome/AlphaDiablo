...
def main():
    log({"event": "arm_a_launch", "arm": ARM, "total_steps": TOTAL_STEPS,
         "step_law": "T=round2048(266240/(1-0.383)),软顶已越,主席步数特批",
         "lever": "worker-dive-action11-logit-bonus=2.0(G0-2 验收 1.72/10)"})
    ...
    for name, mgr, view, pool, registration in exams:
        ...
        if name.startswith("xdevil"):
            ...
            if result["sha16"] == DEVIL_ANCHOR_SHA[pool]:
                ...
                raise SystemExit(1)
    log({"event": "arm_a_stage_complete", "next": "四门配对对判 + 呈主席终裁"})
