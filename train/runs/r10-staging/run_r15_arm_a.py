...
def main():
    log({"event": "arm_e_launch", "arm": ARM, "total_steps": TOTAL_STEPS,
         "step_law": "T=round2048(266240/(1-0.240)),G0-7 定标(战备门砍不合格下潜,share 自然回落)",
         "lever": "readiness-v1 战备门教练(R15 主案)+ 托管 0.5×d^1.6 + a11 杠杆 2.0"})
    rc = run([PY, "train/train_ppo.py", "--worker", "--algo", "mppo",
              ...
              "--manager-heuristic", "readiness-v1",
    ...
    probes = (
        ("deploy-new-a", new_worker, "2114000-2114127"),
        ("deploy-new-b", new_worker, "2115000-2115127"),
        ("deploy-base-a", RUNS / "r14-arm-b-wage" / "model_candidate.zip",
         "2114000-2114127"),
        ("deploy-base-b", RUNS / "r14-arm-b-wage" / "model_candidate.zip",
         "2115000-2115127"),
    )
    ...
    log({"event": "arm_e_stage_complete", "next": "四门配对对判 + 部署对照 + 呈主席终裁"})
