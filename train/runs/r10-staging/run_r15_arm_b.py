...
def main():
    log({"event": "arm_f_launch", "arm": ARM, "total_steps": TOTAL_STEPS,
         "step_law": "T=round2048(266240/(1-0.240)),G0-7 定标(战备门砍不合格下潜,share 自然回落)",
         "lever": "readiness-v2 战备门教练(修正案一:真实清场逃生)+ 托管 0.5×d^1.6 + a11 杠杆 2.0"})
    rc = run([PY, "train/train_ppo.py", "--worker", "--algo", "mppo",
              ...
              "--manager-heuristic", "readiness-v2",
    ...
    for name, mgr, view, pool, registration in exams:
        ...
        if name.startswith("xdevil"):
            ...
            if result["sha16"] == DEVIL_ANCHOR_SHA[pool]:
                ...
                raise SystemExit(1)
    # R15 主指标:部署形态法证探针(readiness 脚本经理 × 工人,双池全量)
    # ——非考卷,不经档案身份系统;基线 = R14 乙工人同探针。
    probes = (
        ("deploy2-new-a", new_worker, "2114000-2114127"),
        ("deploy2-new-b", new_worker, "2115000-2115127"),
        ("deploy2-base-a", RUNS / "r14-arm-b-wage" / "model_candidate.zip",
         "2114000-2114127"),
        ("deploy2-base-b", RUNS / "r14-arm-b-wage" / "model_candidate.zip",
         "2115000-2115127"),
    )
    for name, zip_path, seeds in probes:
        out = STAGING / f"r15-{name}.json"
        rc = run([PY, "train/runs/r10-staging/probe_r15_deployment.py",
                  str(zip_path), seeds, str(out)],
                 f"probe-r15-{name}.log", 14400)
        if rc != 0:
            log({"event": "PROBE_FAILURE", "probe": name, "rc": rc})
            raise SystemExit(1)
        agg = json.load(open(out))["agg"]
        log({"event": "deploy_probe", "probe": name, **agg})
    log({"event": "arm_f_stage_complete", "next": "四门配对对判 + 部署对照 + 呈主席终裁"})
