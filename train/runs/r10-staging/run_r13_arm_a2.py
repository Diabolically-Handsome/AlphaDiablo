...
def main():
    log({"event": "arm_a2_launch", "arm": ARM, "total_steps": TOTAL_STEPS,
         "step_law": "T=round2048(266240/(1-0.353)),G0-3b 定标;软顶已越,主席步数特批",
         "lever": "a11-bonus=2.0 + depth-shaping-unit=8.0 单调势(G0-3b 验收 1.96/10,主席批甲案)"})
    ...
    for name, mgr, view, pool, registration in exams:
        ...
        if name.startswith("xdevil"):
            ...
            if result["sha16"] == DEVIL_ANCHOR_SHA[pool]:
                ...
                raise SystemExit(1)
    log({"event": "arm_a2_stage_complete", "next": "四门配对对判 + 呈主席终裁"})
