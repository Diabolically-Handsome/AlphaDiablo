...
def main():
    log({"event": "arm_d_launch", "arm": ARM, "total_steps": TOTAL_STEPS,
         "step_law": "T=round2048(266240/(1-0.372)),G0-5 定标;主席步数特批continues",
         "lever": "托管 0.5 + 凸曲线 power=1.6(戊案,hazard 拟合 0.23×1.53^(d-1);主席夜令「挖掘一条能用的曲线」)"})
    ...
    for name, mgr, view, pool, registration in exams:
        ...
        if name.startswith("xdevil"):
            ...
            if result["sha16"] == DEVIL_ANCHOR_SHA[pool]:
                ...
                raise SystemExit(1)
    log({"event": "arm_d_stage_complete", "next": "四门配对对判 + 呈主席终裁"})
