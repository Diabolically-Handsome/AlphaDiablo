...
def main():
    log({"event": "arm_c_launch", "arm": ARM, "total_steps": TOTAL_STEPS,
         "step_law": "T=round2048(266240/(1-0.372)),G0-5 定标;主席步数特批continues",
         "lever": "a11-bonus=2.0 + descend-escrow-fraction=0.5 丁案托管(G0-5 验收 2.24/10,主席夜间条例)"})
    ...
    for name, mgr, view, pool, registration in exams:
        ...
        if name.startswith("xdevil"):
            ...
            if result["sha16"] == DEVIL_ANCHOR_SHA[pool]:
                ...
                raise SystemExit(1)
    log({"event": "arm_c_stage_complete", "next": "四门配对对判 + 呈主席终裁"})
