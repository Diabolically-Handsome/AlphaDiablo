...
echo "=== R11-TRIAL train start $(date +%T) ===" | tee -a "$OUT/r11-trial.log"
timeout 28800 $PY train/train_ppo.py --options --algo mppo --gamma 1.0 \
  ...
  --manager-policy-observation-view raw-v4 --reward-economy v3 \
  --run-name r11-trial-killlock --ent-coef 0.02 --lr 3e-4 --seed 22 \
  >> "$OUT/train-r11-trial.log" 2>&1
RC=$?
echo "=== train rc=$RC $(date +%T) ===" | tee -a "$OUT/r11-trial.log"
[ $RC -ne 0 ] && exit $RC
$PY train/export_manager_npz.py \
  train/runs/r11-trial-killlock/model_final.zip \
  train/runs/r11-trial-killlock/policy.npz >> "$OUT/r11-trial.log" 2>&1 || exit 1
for P in a b; do
  if [ "$P" = a ]; then SEEDS=2114000-2114127; else SEEDS=2115000-2115127; fi
  timeout 7200 $PY train/eval_assembled.py --worker "$WZ" \
    --manager-npz train/runs/r11-trial-killlock/policy.npz \
    --manager-policy-observation-view raw-v4 \
    --seeds "$SEEDS" --tag "r11-trial-$P" --reward-economy v3 \
    >> "$OUT/r11-trial-eval-$P.log" 2>&1
  echo "=== eval $P rc=$? $(date +%T) ===" | tee -a "$OUT/r11-trial.log"
done
echo "=== R11-TRIAL complete $(date +%T) ===" | tee -a "$OUT/r11-trial.log"
